...
const state = vi.hoisted(() => ({ reference: null as { id: string } | null, submission: null as any, updates: [] as string[], resolveInput: null as any, tenantCreate: null as any, tenantSelection: null as any, tenantUpdate: null as any, configurationUpdate: null as any, referenceDataCreate: null as any, formCreate: null as any }));
...
vi.mock("../tenantDb", () => ({
  ...
  listTenantForms: vi.fn(async () => []), createTenantForm: vi.fn(async (input: any) => { state.formCreate = input; return { id: "form-new", version: 1 }; }), getTenantSyncBundle: vi.fn(), getTenantDashboard: vi.fn(async () => ({ summary: {}, evolution: [] })), listTenantConflictCases: vi.fn(async () => []),
...
describe("isolation multi-tenant tRPC", () => {
  beforeEach(() => { state.reference = null; state.submission = null; state.updates = []; state.resolveInput = null; state.tenantCreate = null; state.tenantSelection = null; state.tenantUpdate = null; state.configurationUpdate = null; state.referenceDataCreate = null; state.formCreate = null; });
