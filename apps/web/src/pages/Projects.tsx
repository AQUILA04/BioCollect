...
function ProjectsContent() {
  ...
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!tenantId) return toast.error(t("projects.selectTenant")); createProject.mutate({ tenantId, name, description: description || undefined, requiredFingers, nfiqThreshold, matchingThreshold }); }

  return <>
    <PageHeader eyebrow={t("projects.eyebrow")} title={t("projects.title")} description={t("projects.description")} action={canManage ? <Button onClick={() => setShowCreate(true)}><FolderPlus className="mr-2 h-4 w-4" />{t("projects.newProject")}</Button> : undefined} />
    {!tenantId ? <Card><CardContent className="py-12 text-center text-sm text-slate-500">{t("projects.selectWorkspace")}</CardContent></Card> : null}
    {showCreate ? <Card className="mb-6 border-blue-200"><CardContent className="pt-6"><form onSubmit={submit} className="grid gap-5"><div className="grid gap-2"><Label htmlFor="project-name">{t("projects.projectName")}</Label><Input id="project-name" value={name} onChange={event => setName(event.target.value)} minLength={3} required /></div><div className="grid gap-2"><Label htmlFor="project-description">{t("projects.projectDescription")}</Label><Textarea id="project-description" value={description} onChange={event => setDescription(event.target.value)} /></div><fieldset className="grid gap-3"><legend className="text-sm font-medium">{t("projects.requiredFingers")}</legend><div className="flex flex-wrap gap-2">{FINGERS.map(finger => <Button key={finger} type="button" variant={requiredFingers.includes(finger) ? "default" : "outline"} size="sm" onClick={() => toggleFinger(finger)}>{finger}</Button>)}</div></fieldset><div className="grid gap-4 sm:grid-cols-2"><div className="grid gap-2"><Label htmlFor="nfiq">{t("projects.nfiqThreshold")}</Label><Input id="nfiq" type="number" min="1" max="5" value={nfiqThreshold} onChange={event => setNfiqThreshold(Number(event.target.value))} /></div><div className="grid gap-2"><Label htmlFor="matching">{t("projects.matchingThreshold")}</Label><Input id="matching" type="number" min="1" max="100" value={matchingThreshold} onChange={event => setMatchingThreshold(Number(event.target.value))} /></div></div><div className="flex justify-end gap-3"><Button type="button" variant="outline" onClick={() => setShowCreate(false)}>{t("common.cancel")}</Button><Button type="submit" disabled={createProject.isPending || !requiredFingers.length}>{createProject.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}{t("projects.createProject")}</Button></div></form></CardContent></Card> : null}
    {configurationProjectId ? <Card className="mb-6"><CardContent className="pt-6"><div className="mb-5 flex items-start justify-between"><div><p className="text-xs font-semibold uppercase tracking-wider text-blue-700">{t("projects.biometricConfiguration")}</p><h2 className="mt-1 text-lg font-semibold">{configuration.data?.project.name ?? t("projects.loading")}</h2></div><Button variant="ghost" size="sm" onClick={() => setConfigurationProjectId(null)}>{t("projects.close")}</Button></div>{configuration.isLoading ? <div className="text-sm text-slate-500"><Loader2 className="mr-2 inline h-4 w-4 animate-spin" />{t("projects.loading")}</div> : null}{configuration.data?.config ? <div className="grid gap-5"><div className="flex flex-wrap gap-2">{FINGERS.map(finger => <Button key={finger} type="button" variant={requiredFingers.includes(finger) ? "default" : "outline"} size="sm" disabled={!canManage} onClick={() => toggleFinger(finger)}>{finger}</Button>)}</div><div className="grid gap-4 sm:grid-cols-2"><div className="grid gap-2"><Label htmlFor="edit-nfiq">{t("projects.nfiqThreshold")}</Label><Input id="edit-nfiq" type="number" min="1" max="5" value={nfiqThreshold} disabled={!canManage} onChange={event => setNfiqThreshold(Number(event.target.value))} /></div><div className="grid gap-2"><Label htmlFor="edit-matching">{t("projects.matchingThreshold")}</Label><Input id="edit-matching" type="number" min="1" max="100" value={matchingThreshold} disabled={!canManage} onChange={event => setMatchingThreshold(Number(event.target.value))} /></div></div>{canManage && tenantId ? <div className="flex justify-end"><Button onClick={() => updateConfiguration.mutate({ tenantId, projectId: configurationProjectId, requiredFingers, nfiqThreshold, matchingThreshold })} disabled={updateConfiguration.isPending}>{t("projects.save")}</Button></div> : null}</div> : null}</CardContent></Card> : null}
    ...
    {projects.data?.length === 0 ? <Card className="border-dashed"><CardContent className="flex min-h-56 flex-col items-center justify-center text-center"><Settings2 className="mb-4 h-8 w-8 text-blue-600" /><h2 className="font-semibold">{t("projects.noProjects")}</h2><p className="mt-2 max-w-sm text-sm text-slate-500">{t("projects.noProjectsDescription")}</p></CardContent></Card> : null}
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{projects.data?.map(project => <Card key={project.id}><CardContent className="p-5"><div className="flex items-start justify-between gap-3"><div><h2 className="font-semibold text-slate-950">{project.name}</h2><p className="mt-2 min-h-10 text-sm text-slate-500">{project.description || t("projects.noDescription")}</p></div><Badge variant="outline" className={project.isActive ? "border-emerald-200 bg-emerald-50 text-emerald-700" : ""}>{project.isActive ? t("projects.active") : t("projects.inactive")}</Badge></div><div className="mt-5 flex items-center justify-between border-t pt-4"><span className="text-xs text-slate-500">{formatDate(project.createdAt)}</span><Button size="sm" variant="ghost" onClick={() => setConfigurationProjectId(project.id)}>{t("projects.configuration")}</Button></div></CardContent></Card>)}</div>
  </>;
}
