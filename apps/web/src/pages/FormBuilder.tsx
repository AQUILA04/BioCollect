...
const PrefixListInput = forwardRef<
  { commit: () => string[] },
  {
    id: string;
...
>(function PrefixListInput({
  ...
  const [raw, setRaw] = useState(() => prefixText(value));

  useEffect(() => {
    setRaw(prefixText(value));
  }, [value?.join("\0")]);

  function commit(nextRaw = raw) {
    const parsed = parsePrefixes(nextRaw);
    const normalized = prefixText(parsed);
    setRaw(normalized);
    onChange(parsed);
    return parsed;
  }

  useImperativeHandle(ref, () => ({ commit }), [raw, onChange]);
...
function FormBuilderContent() {
  ...
  const defaultPrefixRef = useRef<{ commit: () => string[] }>(null);
