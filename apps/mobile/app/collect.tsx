...
function FieldInput({ field, value, onChange, attachment, onAttach }: { field: FormField; value: string; onChange: (value: string) => void; attachment?: BiometricAttachment; onAttach: () => void }) {
  ...
  return <View style={{ gap: 8 }}><Text style={styles.label}>{field.label}{field.required ? " *" : ""}</Text><TextInput value={value} onChangeText={onChange} placeholder={field.type === "date" ? t("mobile.datePlaceholder") : t("mobile.responsePlaceholder")} keyboardType={field.type === "phone" ? "number-pad" : field.type === "email" ? "email-address" : "default"} autoCapitalize={field.type === "email" ? "none" : "sentences"} placeholderTextColor="#8A98A8" style={styles.input} />{!validation.valid ? <Text style={styles.danger}>{t("mobile.invalidValue", { field: field.label })}</Text> : null}</View>;
}

export default function CollectScreen() {
  const { projectId, formId, campaignId } = useLocalSearchParams<{ projectId: string; formId: string; campaignId: string }>();
